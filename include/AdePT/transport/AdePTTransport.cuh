...
#include <AdePT/transport/kernels/gammas_split.cuh>
#else
#include <AdePT/transport/kernels/electrons.cuh>
#include <AdePT/transport/kernels/gammas.cuh>
#endif
...
__global__ void FinishIteration(AllParticleQueues all, Stats *stats, TracksAndSlots tracksAndSlots)
{
  if (blockIdx.x == 0) {
    ...
    for (int i = threadIdx.x; i < GPUQueueIndex::NumSpecies; i += blockDim.x) {
    ...
    // is needed
    particlesInFlight += all.queues[GPUQueueIndex::GammaWDT].nextActive->size();
...
{
  for (int i = 0; i < GPUQueueIndex::NumParticleQueues; i++) {
    all.queues[i].initiallyActive->clear();
